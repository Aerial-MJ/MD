...
self.logger = logging.getLogger(
    f"verifiers.envs.{self.__class__.__name__}"
)
```

### logging.getLogger 是什么？

它不是“新建 logger”，而是：

> **按名字“获取（或创建）一个 logger 实例”**
...
```
logging.getLogger("verifiers.envs").setLevel(logging.DEBUG)
```
...
- 如果是 `BankEnv` → logger 名就是 `verifiers.envs.BankEnv`
- 如果是 `TaxEnv` → 自动变成 `verifiers.envs.TaxEnv`

👉 **子类完全不用改 logging 代码**

这是**抽象基类里写 logger 的标准姿势**
...
```
class BankEnv(Environment):
    def step(self, action):
        self.logger.debug(f"action received: {action}")
```

输出会像：

```
[DEBUG] verifiers.envs.BankEnv action received: ...
```
