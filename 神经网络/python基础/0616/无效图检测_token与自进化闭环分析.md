...
| **一致性** | 和训练用同一个 [apply_chat_template]，确保 prompt 格式完全一致 |
...
分三步：**从 JSON 读数据 → 构建 messages → apply_chat_template 生成文本 prompt**

### 第 1 步：原始数据长什么样

测试集 `ig_sft_v4_test.json` 是 ShareGPT 格式，每条数据大概这样：

```json
{
  "messages": [
    {"role": "system", "content": "你是一个图片判定助手..."},
    {"role": "user", "content": "<image>\n请判断这张图片是否有效"},
    {"role": "assistant", "content": "{\"label\": \"无效图\", \"reason\": \"...\"}"}
  ],
  "images": ["/path/to/image.jpg"]
}
```

### 第 2 步：[build_messages](file:///home/hadoop-grocery-rc/dolphinfs_hdd_hadoop-grocery-rc/yxfk/work_dir/chenjiarui/work/IG/SFT/LlamaFactory/eval/eval_ig_vllm.py#70#4) 转换格式
...
for m in messages:
    if m["role"] == "assistant":
        continue                    # ① 推理时不要 assistant 轮（模型自己生成）

    if role == "user" and has_image:
        clean_text = text.replace("<image>", "").strip()  # ② 去掉 <image> 占位符
        content = [
            {"type": "image"},                              # ③ 图片占位符
            {"type": "text", "text": clean_text}            # ④ 文本内容
        ]
    else:
        content = text                                      # ⑤ system 轮直接用文本
```
...
[
    ...
    {"role": "user", "content": [
        {"type": "image"},                        # 图片位置标记
        {"type": "text", "text": "请判断这张图片是否有效"}  # 文本
    ]}
]
```

注意：`{"type": "image"}` 只是告诉模板"这里有个图片"，**实际图片数据不在这里**，后面单独传入。

### 第 3 步：[apply_chat_template 生成最终文本 prompt

```python
text_prompt = processor.apply_chat_template(
    messages,
    tokenize=False,          # 不分词，只生成文本
    add_generation_prompt=True,  # 末尾加 <|im_start|>assistant\n
)
```

生成的 text_prompt 大概长这样：
...
其中 `<|vision_start|><|image_pad|><|vision_end|>` 就是 `{"type": "image"}` 被模板渲染出来的，是图片 token 的占位符。

### 第 4 步：图片数据单独挂载

```python
img = load_and_resize_image(img_paths[0], args.image_max_pixels)  # PIL Image 对象

prompts.append({
    "prompt": text_prompt,                      # 文本 prompt（含 <|image_pad|> 占位符）
    "multi_modal_data": {"image": img},          # 实际图片数据（PIL Image）
})
```
...
```
text_prompt 里的 <|image_pad|>  ←→  multi_modal_data 里的 PIL Image
        ↑                                    ↑
   告诉模型"这里有图"              提供图片的实际像素数据
   （占位符，决定 token 位置）        （vLLM 会编码成 vision tokens 填进去）
```
...
这里的 batch_size 不是 GPU 层面的 micro-batch，而是**一次传给 vLLM 引擎的样本条数**：

```python
batch_size = args.batch_size    # 默认 64
...
假设测试集有 500 条数据，batch_size = 64：

```
总批次数 = (500 + 64 - 1) // 64 = 563 // 64 = 8 批

批次 0: 第   0 ~  63 条
批次 1: 第  64 ~ 127 条
批次 2: 第 128 ~ 191 条
...
批次 7: 第 448 ~ 499 条（最后一批不满 64）
```

### vLLM 内部怎么处理这 64 条

你传 64 条进去，vLLM **不会**等 64 条全跑完才返回，它内部有自己的调度：

```
你传入 64 条
    │
    ▼
vLLM 内部 continuous batching：
  - 按显存情况，同时并行处理若干条（可能 20~40 条）
  - 一条跑完就释放显存，立刻塞入下一条
  - 全部跑完后统一返回 64 条的结果
```

所以 `batch_size=64` 的意思是**一次提交 64 条任务给 vLLM**，vLLM 内部自己调度并行度，你不需要管 GPU 实际同时跑几条。

### batch_size 怎么选

| batch_size | 优点 | 缺点 |
|-----------|------|------|
| 太小（如 8） | 显存安全 | GPU 利用率低，总耗时长 |
| 太大（如 512） | 提交次数少 | 可能 OOM（显存不够） |
| **64（默认）** | 平衡点 | 大多数情况够用 |

**调大**：如果 GPU 显存充足（如 A100 80G 跑 2B 模型），可以调到 128 甚至 256。  
**调小**：如果报 OOM 或超长样本多，降到 32。
...
```
ig_sft_v4_test.json（ShareGPT 格式）
    │
    ▼  build_messages()
messages 列表（system + user，去掉 assistant 轮）
    │
    ▼  apply_chat_template()
text_prompt（含 <|image_pad|> 占位符的纯文本）
    │
    ▼  加载图片 + resize
PIL Image 对象
    │
    ▼  组合
{"prompt": text_prompt, "multi_modal_data": {"image": img}}
    │
    ▼  按 batch_size=64 分批
批次 0: [prompt_0, ..., prompt_63]
批次 1: [prompt_64, ..., prompt_127]
...
    │
    ▼  llm.generate(batch, sampling_params)
vLLM 内部 continuous batching 并行推理
    │
    ▼
返回每条的生成文本
```