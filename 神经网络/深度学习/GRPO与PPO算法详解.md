...
### GAE 的两个极端：λ=0 是单步 TD，λ=1 是蒙特卡洛

GAE 公式里的 $\lambda$ 在两个端点上会分别退化成强化学习中最基础的两种估计方式，把这两种情况推一遍，能更直观地理解 $\lambda$ 到底在"折中"什么：

**λ=0：退化为单步 TD**

$$A_t^{GAE(\gamma,0)} = \delta_t = r_t + \gamma V(s_{t+1}) - V(s_t)$$

只用一步的真实奖励 $r_t$，之后全部依赖 Critic 对 $V(s_{t+1})$ 的估计（这叫"自举" bootstrap）。方差最小（只有一个随机变量 $r_t$），但如果 $V$ 估计不准，偏差也会直接带入 $A_t$。

**λ=1：退化为蒙特卡洛（MC）**

把 GAE 公式展开，$\delta_{t+k} = r_{t+k} + \gamma V(s_{t+k+1}) - V(s_{t+k})$，代入 $\lambda=1$：

$$
A_t^{GAE(\gamma,1)} = \sum_{k=0}^{T-t} \gamma^k \delta_{t+k}
= \sum_{k=0}^{T-t} \gamma^k \big(r_{t+k} + \gamma V(s_{t+k+1}) - V(s_{t+k})\big)
$$

按 $k$ 展开后，中间的 $V(s_{t+k+1})$ 项和下一项的 $-V(s_{t+k+1})$ 项两两相消（望远镜求和 / telescoping sum），最终只剩下：

$$
A_t^{GAE(\gamma,1)} = \left(\sum_{k=0}^{T-t} \gamma^k r_{t+k}\right) - V(s_t) = G_t - V(s_t)
$$

也就是说，$\lambda=1$ 时 GAE 完全不依赖中间的 $V(s_{t+1}), V(s_{t+2}), \dots$（它们全部抵消掉了），只剩下**真实观测到的蒙特卡洛回报 $G_t$**（定义与 [MiniMind从0到1构建大模型 · 蒙特卡洛回报](../神经网络代码/MiniMind从0到1构建大模型.md#43-强化学习) 中的 $G_t$ 完全一致）减去当前状态的 baseline $V(s_t)$。这正是"蒙特卡洛方法对梯度计算的作用"的数学落点：

- **无偏**：$G_t$ 是真实奖励序列的加权和，不掺杂任何 Critic 的估计误差，因此 $A_t^{GAE(\gamma,1)}$ 是真实 advantage 的无偏估计；
- **高方差**：$G_t$ 要把 $t$ 之后所有步骤的随机性都累加起来，轨迹越长、奖励越稀疏，方差越大；
- $\lambda$ 从 0 到 1 连续变化，本质上是在"单步 TD（低方差、有偏）"与"蒙特卡洛（无偏、高方差）"之间做加权插值，GRPO 用组内 reward 归一化代替 Critic，可以看作是完全放弃 Critic、直接使用（组内标准化后的）蒙特卡洛式回报的一种极端选择。
