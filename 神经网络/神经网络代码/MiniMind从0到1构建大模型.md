...
**这是 RoPE 的通用规律，不是 MiniMind 专属的实现细节**

只要一个模型用的是 RoPE（而不是绝对位置编码），下面这套流程和位置是通用的，LLaMA、Qwen、Mistral、Gemma 等主流模型都遵循同样的顺序：

```
Token Embedding（查表）→ x                    ← ❌ 这里没有 RoPE，x 不含任何位置信息
   ↓
每一层 Attention 内部（重复 N 次，每层各自独立执行）：
   q = q_proj(x)，k = k_proj(x)，v = v_proj(x)  ← 线性投影，此时都不带位置信息
   q = q_norm(q)，k = k_norm(k)                 ← （若有 QK-Norm）先做，且只 norm q、k，不 norm v
   q, k = apply_rotary_pos_emb(q, k, cos, sin)  ← ✅ RoPE 在这里发生，只旋转 q、k
   scores = q @ k.T / sqrt(d)                   ← 用旋转后的 q、k 计算注意力分数
   output = softmax(scores) @ v                 ← v 全程未被旋转，只在这一步被加权求和
```

三个关键结论：

1. **RoPE 绝不发生在 Embedding 层**：Embedding 输出的 `x` 永远是纯语义向量，不含位置信息。这是 RoPE（相对位置编码）和"绝对位置编码"（原始 Transformer 的正弦编码、GPT-2 的可学习位置 Embedding，`x = token_emb + position_emb` 只在最开始加一次）最本质的结构性区别。
2. **RoPE 只对 Query 和 Key 生效，Value 永远不参与旋转**：因为 RoPE 的数学目的是让 $q^\top k$ 的点积自然带上"相对位置"信息（旋转角度差 $=(n-m)\theta$），只有参与点积运算的 Q、K 需要旋转；V 只是被注意力权重加权求和，不需要也不应该旋转。
3. **RoPE 在每一层都要重新执行一次**，不是像 Embedding 那样只做一次然后全局共享——虽然 `cos`/`sin` 这张角度表是全局预计算、跨层复用的，但"用这张表去旋转 Q、K"这个操作，每一层的 Attention 都要用当层新算出来的 Q、K 重新做一遍。

**一个需要注意的例外：MLA 架构（如 DeepSeek 系列）**

MLA（Multi-head Latent Attention）会把大部分 K/V 压缩进一个低秩隐向量以节省 KV Cache，但 RoPE 的旋转矩阵是随位置变化的，如果直接对被压缩的低秩部分做旋转，会破坏"权重矩阵可以离线合并（矩阵吸收）"的性质。因此 DeepSeek 的做法是**单独切出一小部分不参与低秩压缩的维度专门承载 RoPE**（decoupled RoPE），压缩的主体部分则不做旋转。这仍然符合"RoPE 只作用于 Q、K（用于计算注意力分数的部分），不作用于 V"的核心原则，只是在 Q、K 内部又进一步区分了"参与旋转的维度"和"参与低秩压缩的维度"。
...
上面 $G_t$ 的计算方式，本质上是强化学习中的**蒙特卡洛（Monte Carlo，MC）方法**：不依赖任何环境动态模型，直接让当前策略完整跑完一条轨迹（这里是完整生成一个回答），再用观测到的真实奖励序列 $r_1, \dots, r_T$ 反推每个 token 的回报 $G_t$。它对策略梯度估计的作用可以归纳为三点：

1. **无偏性来自"用真实回报代替期望回报"。** 4.3.1 节末尾的数学推导给出 $\nabla_\theta J(\theta) = \mathbb{E}_{y\sim\pi_\theta}\big[R(y)\nabla_\theta \log\pi_\theta(y|x)\big]$，这是一个关于 $\pi_\theta$ 的期望，无法直接计算。蒙特卡洛方法用"采样一条轨迹、代入它的真实回报 $G_t$"来做无偏估计：只要采样动作服从当前策略 $\pi_\theta$（on-policy），单条轨迹算出的 $G_t \nabla_\theta\log\pi_\theta(a_t|s_t)$ 就是梯度的一个无偏样本，多条轨迹取平均后方差降低、但期望不变、仍收敛到真实梯度。这一点不依赖任何值函数近似，是它相对 TD/GAE 类方法"无偏但高方差"这一特性的来源（可与 [GRPO与PPO算法详解 · 五、PPO 的 Advantage：V(s)、δ 和 GAE](../深度学习/GRPO与PPO算法详解.md#五ppo-的-advantagevsδ-和-gae) 中"MC 是 GAE 在 $\lambda=1$ 时的极限情况"对照阅读）。
2. **必须等完整轨迹结束才能计算，因此天然适合 episodic、有明确终止信号的任务。** 语言模型生成一个回答直到 EOS，正是一次完整的 episode；规则验证器也只能在生成结束后才能判断对错。这决定了 $G_t$ 只能在整条回答生成完毕、拿到最终奖励后，从后向前递推得到（见上面的代码），不能像 TD 方法那样每生成一个 token 就立即更新。
3. **代价是方差较大，尤其当轨迹长、奖励稀疏时。** 一条轨迹的回报 $G_t$ 本身是一个随机变量（不同采样得到的回答千差万别），用单条或少量轨迹的 $G_t$ 去估计期望梯度，方差会比引入价值函数 baseline（如 PPO 的 Critic、GRPO 的组内均值）大得多——这也是 PPO 引入 GAE、GRPO 引入组内 reward 归一化的核心动机：它们都是在"蒙特卡洛的无偏但高方差"与"自举估计的有偏但低方差"之间做折中。

> 需要澄清两个常见的误解：① 蒙特卡洛方法的探索性来自策略本身的随机采样（例如这里的 softmax 分布，或标准 RL 教材中的 $\varepsilon$-贪婪），但 $\varepsilon$-贪婪主要用于 Q-learning 等基于值函数的方法，纯策略梯度方法通常不需要额外叠加 $\varepsilon$-贪婪。② 蒙特卡洛方法要求**在一个 episode 内**等到轨迹结束才能计算回报，但这仍然是**on-policy** 的在线学习（每次用当前最新策略采样、更新），并不等同于"离线强化学习（offline RL，从固定历史数据集里学习、完全不与环境交互）"——"等轨迹结束再更新"和"离线任务"是两个不同的概念，不能混为一谈。
...
> 关联笔记：熵一旦失控下降到接近 0（熵崩塌）会导致策略丧失探索能力，其物理/化学/信息论多角度成因与工程排查手段见 [熵崩塌：跨学科视角与GRPO工程实践](../深度学习/熵崩塌：跨学科视角与GRPO工程实践.md)。
...
在整条轨迹结束后计算一次 reward，按是否调用工具分两条分支：

- 分支 A：不调用工具 —— 无 ground truth 的数据，使用奖励模型 Internlm2-1.8b-reward 打分；
- 分支 B：调用工具 —— 有 ground truth 的数据，使用规则奖励。

最终所有奖励的尺度 clip 到 $[-3, 3]$，具体奖励设计如下表：
...
| 无工具 | 长度分 | `+0.5` / `-0.5` |
| 无工具 | RM 打分 | `+score` |
| 有工具 | 工具对齐分 | `+0.5` 或 `-0.5 × tool_gap` |
| 有工具 | GT 命中分 | `+2.5 × 命中数 / len(gt)` |
| 有工具 | 未完成扣分 | `-0.5` |
...
##### 辨析：混合精度训练到底占用多少显存？是模型参数量的多少倍？

这是混合精度训练最实用、也最容易讲混的一个问题。下面以最常见的 **Adam/AdamW 优化器 + fp16/bf16 混合精度** 为例，设模型参数量为 $\Psi$，把参数、梯度、优化器状态这三大块的显存占用拆开算清楚（不含激活值/KV Cache，那部分和 batch size、序列长度强相关，见 [分布式训练与推理加速全解](分布式训练与推理加速全解.md)）。

**① 模型参数：混合精度下几乎总是"两份都占"，不是"只占一半"**

- fp16/bf16 权重（前向反向计算用）：$2\Psi$ 字节
- fp32 主参数副本（master weights，优化器内部持有，真正被更新的是这一份）：$4\Psi$ 字节

参数更新公式是 $\theta \leftarrow \theta - \eta \cdot g$，本质是"大数减一个相对很小的数"。fp16 只有 10 位尾数：

```
假设 θ = 1.0，学习率 η = 0.001，梯度 g = 0.1，更新量 = η × g = 0.0001
fp16 在 1.0 附近的最小可分辨间隔（ULP）约为 0.0009765625，比更新量还大！
若全程只用 fp16 存 θ：0.9999 会被舍入回 1.0，更新量直接被"吃掉"，模型学不动
若用 fp32 存 θ（主参数副本）：23 位尾数精度足够，更新被正确保留
```

所以严格来说，混合精度训练里**参数这一项显存不降反升**：从纯 fp32 训练的 $4\Psi$ 涨到 $2\Psi + 4\Psi = 6\Psi$。这是"用显存换计算速度和数值稳定性"的权衡，混合精度真正省的是前向/反向计算时的激活值显存和算力时间，不是参数本身的显存。

**② 梯度：这是最容易讲错的一项，实际取值取决于训练框架的工程实现，而非"计算是否需要 fp32"**

首先要明确一点：**优化器更新参数时，梯度参与运算几乎总是需要 fp32 精度**，这是共性，没有争议。但"是否会因此常驻多一份 fp32 梯度张量"，是完全取决于工程实现的，会导致梯度显存出现三种不同的典型取值：

| 梯度存储方式 | 显存占用 | 典型代表 |
|---|---|---|
| 只有 fp16 梯度常驻，`optimizer.step()` 内部逐元素临时 cast 成 fp32 参与计算，算完立刻丢弃，不生成新的常驻张量 | $2\Psi$ | 朴素 PyTorch AMP（无梯度累积的单卡训练） |
| fp16 梯度（反向传播直接产出）+ 额外的 fp32 累加 buffer 共存，用于跨 micro-batch 的梯度累积或跨卡同步 | $2\Psi + 4\Psi = 6\Psi$ | 部分梯度累积（gradient accumulation）实现 |
| 只有 fp32 梯度，通过 `main_grad` 机制或 `reduce_dtype=fp32` 配置，反向传播的 kernel 直接把低精度算出的梯度就地累加写入预先分配好的 fp32 buffer，从不单独产生一份实际存储的 fp16 梯度张量 | $4\Psi$ | Megatron-LM 的 `main_grad`、PyTorch FSDP 配置 `MixedPrecision(reduce_dtype=torch.float32)`、DeepSpeed ZeRO 的常见工程实现 |

**关键澄清**：第一种里"cast 成 fp32"这个动作确实每一步都会发生，但那是计算 kernel 内部的临时寄存器级转换，算完即弃，并不会让显存中同时出现"一份 fp16 梯度 + 一份新分配的 fp32 梯度"；这就好比手里只有一张人民币，收银台内部按汇率折算成美元记账，但钱包里始终只有那一张人民币，不会凭空多出一张美元钞票。而**工业界大规模训练框架（Megatron-LM、FSDP、DeepSpeed）为了避免"两份都占显存"的浪费，同时又要保证跨 micro-batch/跨卡累积的精度，往往会直接选择第三种方案**：从训练一开始就只维护一份常驻的 fp32 梯度 buffer，低精度梯度算出来的瞬间就地累加进去，中间态极短暂、可忽略，所以**在工业界实践中，梯度显存"常驻 $4\Psi$、不需要额外 cast 出一份独立张量"是相当常见的做法**，比朴素实现的 $2\Psi$ 或部分实现的 $6\Psi$ 更节省、更稳妥。

**③ 优化器状态：Adam/AdamW 的真正大头，几乎总是 fp32**

- 一阶矩 $m$（momentum）：$4\Psi$ 字节
- 二阶矩 $v$（variance）：$4\Psi$ 字节

为了数值稳定性，这两项几乎总是保持 fp32，不论 fp16 还是 bf16 训练都是如此——这也是为什么即使用 bf16 训练，优化器状态依然是显存大头的原因。

**④ 汇总：混合精度 + Adam 训练，显存约为参数量的 10~16 倍（不含激活值）**

以最常见、DeepSpeed ZeRO 论文里给出的经典估算（梯度取 $2\Psi$，只在 `optimizer.step()` 时临时 cast）为基准：
...
即 **显存(不含激活值) ≈ $16\Psi$ 字节 = 模型参数量的 16 倍**，每 1B 参数约需 16GB 显存（不含激活值/KV Cache）。这是业界最常引用的估算基准，但不是唯一取值，常见变体：

- **纯 bf16 端到端训练（不保留 fp32 主参数副本）**：$2+2+4+4=12\Psi$，约 **12 倍**——但如前所述，工业界大规模预训练框架为了长期训练的数值稳定性，很多时候仍然选择保留 fp32 主参数副本，这个"省法"更多见于小规模/实验性训练。
- **梯度用 $4\Psi$（main_grad / reduce_dtype=fp32 机制，权重仍是 $2\Psi+4\Psi$）**：$2+4+4+4+4=18\Psi$，约 **18 倍**——这是工业界大规模训练框架里相当常见的取值，虽然单看梯度这一项显存更大，但换来的是不需要额外的 fp16 梯度张量共存、也不需要每步都做一次显式的整体 cast。
- **8-bit optimizer（如 bitsandbytes 的 Adam8bit）**：把优化器状态压缩到 8bit，可降到约 $2+4+2+1+1\approx10\Psi$。
- **纯 fp32 训练（无混合精度）**：$4\Psi$(权重) + $4\Psi$(梯度) + $8\Psi$(优化器状态) = $16\Psi$——注意这和标准混合精度公式数值上恰好相近，但构成完全不同（混合精度是"多份精度换速度"，纯 fp32 是"单份高精度但算得慢"）。
- **SGD（无动量）**：优化器状态几乎为 0，总显存约 $2\Psi$(权重)+$2\Psi$(梯度)$\approx4\Psi$，远小于 Adam。

**一句话总结：** 混合精度训练不是"整体降精度省显存"，而是"多份精度按需共存换速度"，参数、梯度这两项在混合精度下常常比纯 fp32 训练占用更多显存；具体倍数没有唯一标准答案，需要看梯度和主参数副本的工程实现方式，**10~18 倍参数量都是实践中真实出现过的取值，其中 16 倍是最常被引用的经典估算基准**，优化器状态（Adam 的 $8\Psi$）几乎总是显存里最大且最不容易压缩的一块。

**⑤ bf16 还算"混合精度"吗？相比 fp16，能确定省掉什么、不能确定省掉什么**

当前大模型训练主流用的是 **bf16 autocast**，而不是 fp16 + Loss Scaling 这套更复杂的机制，但 bf16 通常仍然归为"混合精度"训练（autocast 内部依然有 softmax、归约等少数算子会自动转到 fp32 计算）。它相比 fp16，省的东西没有想象中那么多，要分两部分看：

**能确定省掉的：Loss Scaling 整套机制**

fp16 的指数只有 5 位，数值范围窄，很多真实梯度值（比如 1e-7）表示不了会直接下溢成 0，所以必须靠 `GradScaler` 把 loss 放大再算梯度、之后再缩小还原。bf16 的指数位数和 fp32 完全一样（都是 8 位），数值范围与 fp32 相同，基本不会出现这种下溢问题，因此：

- 不需要 `GradScaler`，直接 `loss.backward()` 即可，省掉 `scaler.scale()` / `scaler.unscale_()` / `scaler.update()` 这一整套逻辑；
- 对应到显存上，省掉的是 Loss Scaling 相关的少量状态开销（scale factor 及其动态调整的记录），这部分本身占用不大，**核心价值是简化了训练管线、避免了 fp16 训练中常见的"loss 变 nan/inf 需要调 scale"的调试成本**，而不是显存上的大头。

**不能一概而论的：fp32 主参数副本**

这是最容易被过度简化的一点，**不能说"bf16 就一定不需要 fp32 主参数副本"**：

- bf16 尾数只有 7 位（比 fp16 的 10 位还少），"大参数减一个小的更新量" $\theta \leftarrow \theta - \eta \cdot g$ 这个舍入误差问题**依然存在**，只是没有 fp16 那种因指数范围窄导致的下溢问题那么致命；
- 理论上，bf16 端到端训练（参数、梯度全程 bf16，不额外维护 fp32 副本）是可行的，一些小规模/实验性训练确实这么做，这种情况下能把参数这一项从混合精度的 $2\Psi+4\Psi=6\Psi$ 降到只有 $2\Psi$；
- 但**工业界主流的大模型预训练框架（DeepSpeed、Megatron-LM 等），即使用 bf16 训练，也往往依然保留 fp32 主参数副本**，因为大模型要训练几十万、上百万步，即便每一步的舍入误差很小，长期累积下来也会明显影响收敛，尤其是训练后期学习率衰减、更新量本身变小的阶段，用 bf16 直接存参数容易出现"更新量被舍入吃掉、后期学不动"的问题。这是经过大量实践验证后的稳妥工程默认选择，不是 bf16 的"必然特性"。

**结论：bf16 相比 fp16，唯一能确定省掉的是 Loss Scaling 这一整套机制**（因为下溢问题的根源是指数范围，bf16 指数范围等于 fp32，这个问题基本不存在了）；**fp32 主参数副本是否保留，取决于具体训练框架和场景，不是 bf16 本身决定的**——小规模/实验性训练可以省掉（降到约 $2+2+4+4=12\Psi$，见上面④中的"纯 bf16 端到端训练"变体），但主流大规模预训练框架通常仍然保留（此时 bf16 相比 fp16 在显存上的唯一差别就只是不用为 Loss Scaling 维护额外状态，其余各项——权重 $2\Psi+4\Psi$、梯度、优化器状态 $8\Psi$——和 fp16 混合精度基本一致）。
...
#### 4.5.5 训练算力估算：为什么是 6ND

规划一次预训练需要多少 GPU、多久能训完，业界有一个经典的近似公式：

$$C \approx 6ND$$

其中：
- $C$：总计算量（单位 FLOPs，浮点运算次数）
- $N$：模型参数量
- $D$：训练用的总 token 数

这个公式来自 Kaplan et al., 2020《Scaling Laws for Neural Language Models》，后来被 Chinchilla（Hoffmann et al., 2022）等工作广泛引用。下面拆开看这个"6"到底是怎么来的。

##### 1）前向传播 ≈ 2N：矩阵乘法本身就是"乘加"操作

容易有一个误解：矩阵乘法只有乘法，"乘加"里的那个"加"是不是指额外的 bias 加法？—— 不是，**加法就藏在矩阵乘法内部**，bias 加法反而是可以忽略的部分。

先看最简单的向量点积：

$$y = w \cdot x = w_1 x_1 + w_2 x_2 + \cdots + w_n x_n$$

这一步包含 $n$ 次乘法 + $n$ 次加法（把 $n$ 个乘积累加起来），一共 $2n$ 次浮点运算。硬件上乘法和加法通常被设计成一个原子操作——MAC（Multiply-Accumulate，乘累加）：`acc += a × b`，这也是为什么算力常用 FLOPs 而不是单独统计乘法次数。

推广到矩阵乘法：设权重矩阵 $W \in \mathbb{R}^{m\times n}$，输入 $x\in\mathbb{R}^n$，输出 $y=Wx \in \mathbb{R}^m$：

```
输出的每一个元素 y_i = 一次向量点积（n次乘法 + n次加法）= 2n 次运算
输出共有 m 个元素
总计：2mn 次浮点运算

而这个矩阵恰好有 m×n 个参数
所以：每个参数贡献 ≈ 2 FLOPs（1次乘法 + 1次加法）
```

举个 $W \in \mathbb{R}^{4\times 3}$ 的具体例子：参数量 $=4\times3=12$，每个输出元素需要 3 次乘法 + 3 次加法，4 个输出元素合计 12 次乘法 + 12 次加法 $=24$ 次运算，正好是 $2\times12=2N$。**这就是 2N 里"2"的来源：矩阵乘法内部乘加结构决定的，和额外的加法层无关。**

**那 bias 的加法用在哪、为什么可以忽略？**

线性层完整公式是 $y = Wx + b$，其中 $b\in\mathbb{R}^m$ 只贡献 $m$ 次加法（没有乘法）。对比权重部分的 $2mn$ 次运算：

$$\frac{m}{2mn} = \frac{1}{2n}$$

当 $n$（比如隐藏层维度 4096）很大时，这个比例小到可以忽略（约 0.01%），所以估算 6ND 时直接只算权重矩阵的 $2N$，bias 的加法量级上完全可以忽略。

##### 2）反向传播 ≈ 4N：两个梯度，形式和前向对称

反向传播时，假设从上游传回来的梯度是 $\delta = \frac{\partial L}{\partial y} \in \mathbb{R}^m$，对于同一个线性层 $y=Wx$，需要算两个量：

**① 对权重的梯度（用来更新参数）：**

$$\frac{\partial L}{\partial W} = \delta \cdot x^T$$

这是一个外积运算，$\delta\in\mathbb{R}^m$、$x\in\mathbb{R}^n$，结果是 $m\times n$ 矩阵，每个元素 $(\partial L/\partial W)_{ij} = \delta_i x_j$ 只需 1 次乘法，没有累加求和，但整体算完 $mn$ 个元素还是 $mn$ 次乘法，考虑到梯度的存储和后续更新一般仍按照 2 FLOPs/参数 估算，计算量同样是 $2mn$，恰好等于前向传播的计算量。

**② 对输入的梯度（用来继续向前一层传播，链式法则要用）：**

$$\frac{\partial L}{\partial x} = W^T \cdot \delta$$

这是一个矩阵向量乘法，形式和前向传播 $y=Wx$ 完全一样（只是矩阵换成了 $W^T$），计算量同样是 $2mn$ FLOPs。
...
|------|------|--------|
| 前向传播 | $y=Wx$ | $2mn$ |
| 反向①：权重梯度 | $\partial L/\partial W = \delta x^T$ | $2mn$ |
| 反向②：输入梯度 | $\partial L/\partial x = W^T\delta$ | $2mn$ |
| **合计** | | $6mn = 6N$ |

反向传播的两个梯度，本质上都是矩阵乘法，形式和前向传播完全对称，所以各自计算量都约等于一次前向，加起来就是 $2\times$ 前向 $=4N$，再加上前向本身的 $2N$，总共 $6N$（每个 token）。

##### 3）乘以总 token 数，得到总算力

```
每 token 计算量：Cforward(2N) + Cbackward(4N) = 6N
乘以训练用的总 token 数 D：
  C ≈ 6ND
```
...
$$C = 6\times175\times10^9\times300\times10^9 \approx 3.14\times10^{23}\ \text{FLOPs}$$

单块 A100（312 TFLOPS）跑约需要 1000 天，实际用约 1000 块 A100 跑了约 1 个月。

**⚠️ 注意事项：**
- $6ND$ 是近似值，忽略了 Attention 里 $O(n^2)$（$n$ 为序列长度）的那部分计算量，序列长度不太长时影响不大
- 实际训练还有通信开销、重计算（activation checkpointing，见 4.5.3 节）等额外消耗，真实算力需求会比 $6ND$ 更高
- 相关论文：Kaplan et al., 2020《Scaling Laws for Neural Language Models》；Hoffmann et al., 2022《Training Compute-Optimal Large Language Models》（Chinchilla）
